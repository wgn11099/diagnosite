...
wgn.timeScroll = null;  //定义整屏切换动画的实例

wgn.currentStep = "step1"; //定义默认当前状态为step1

//初始化配置
wgn.init = function () {
    wgn.resize();//设置每一屏的高度和top值
    wgn.events();//配置事件
    wgn.configIntAnimate();//配置导航条的动画

    wgn.button3D(".start",".state1",".state2",0.3);
    wgn.button3D(".button1",".state1",".state2",0.3);
    wgn.button3D(".button2",".state1",".state2",0.3);

    //设置每一屏中img的百分比
    wgn.imgWidthp( $(".scene img") );

    wgn.configTimeScroll();//配置整屏切换的动画

    twoAnimate.init();  //执行第二屏里面的动画
    threeAnimate.init();//执行第三屏里面的动画
    fiveAnimate.init(); //执行第五屏里面的动画
};

$(document).ready( wgn.init );

//配置事件
wgn.events = function () {


    wgn.nav();//执行导航条的鼠标移入移出的动画
    ...
    //在滚动条滚动的过程中计算页面中应该到哪个时间点上去
    $(window).bind("scroll",wgn.scrollStatus);
    ...
    //当mouseup的时候，让当前这一屏某个状态
    $(window).bind("mouseup",wgn.mouseupFn);
    ...
    function mousewheelFn(ev,direction) {
        $(window).unbind("scroll",scrollFn);//解除定义的滚动事件及处理函数
        if(direction<1){//向下滚动
            wgn.changeStep("next");
        }else{//向上滚动
            wgn.changeStep("prev");
    ...
    $(window).resize( wgn.resize );
};

//当mouseup的时候，让当前这一屏某个状态
wgn.mouseupFn = function () {
    //在滚动过程中计算出的一个比例
    var scale = wgn.scale();
    //得到当前页面到达的某个时间点
    var times = scale * wgn.timeScroll.totalDuration();

    //获取到上一个状态和下一个状态
    var prevStep = wgn.timeScroll.getLabelBefore(times);
    var nextStep = wgn.timeScroll.getLabelAfter(times);

    //获取到上一个状态的时间和下一个状态的时间
    var prevTime = wgn.timeScroll.getLabelTime(prevStep);
    var nextTime = wgn.timeScroll.getLabelTime(nextStep);
    ...
    wgn.timeScroll.tweenTo(step);
    //---------------------当松开鼠标时，控制滚动条到达某个状态计算出来的距离-----------------------------------------//
    //获取动画的总时长
    var totalTime = wgn.timeScroll.totalDuration();

    //获取到要到达的状态的时间
    var afterTime = wgn.timeScroll.getLabelTime(step);
    ...
    //滚动条滚动的距离的持续时间
    var d = Math.abs( wgn.timeScroll.time() - afterTime);

    var scrollAnimate = new TimelineMax();
    scrollAnimate.to("html,body",d,{scrollTop : positionY});//html是为了兼容火狐浏览器

    wgn.currentStep = step;
};
...
//计算在滚动条滚动的过程中计算页面中应该到哪个时间点上去
wgn.scrollStatus = function(){
    var times = wgn.scale() * wgn.timeScroll.totalDuration();
    //当滚动条在滚动的过程中，让页面中的动画到达某个时间点
    wgn.timeScroll.seek(times,false);
};


//切换整屏并且计算滚动条的距离
wgn.changeStep = function (value) {
    if(value === "next"){//向下切换

        //获取当前状态的时间
        var currentTime = wgn.timeScroll.getLabelTime(wgn.currentStep);

        //获取到下一个状态的字符串
        var afterStep = wgn.timeScroll.getLabelAfter(currentTime);
        ...
        //获取到下一个状态的时间
        var afterTime = wgn.timeScroll.getLabelTime(afterStep);
        ...
        //运动到下一个状态
        //wgn.timeScroll.tweenTo(afterStep);

        //记录当前的状态为下一个状态，方便继续切换到下一个状态上
        wgn.currentStep = afterStep;
        ...
        if(value === "prev") {//向上切换
            ...
            //获取到上一个状态的字符串
            var beforeStep = wgn.timeScroll.getLabelBefore(currentTime);
            ...
            //获取到上一个状态的时间
            var beforeTime = wgn.timeScroll.getLabelTime(beforeStep);
            ...
            //滚动条滚动的距离的持续时间
            var d = Math.abs( wgn.timeScroll.time() - beforeTime);

            var scrollAnimate = new TimelineMax();
            scrollAnimate.to("html,body",d,{scrollTop : positionY});//html是为了兼容火狐浏览器

            //运动到上一个状态
            //wgn.timeScroll.tweenTo(beforeStep);

            //记录当前的状态为上一个状态，方便继续切换到上一个状态上
            wgn.currentStep = beforeStep;
        }
    }
};

//配置整屏切换的动画以及每一屏中的小动画
wgn.configTimeScroll = function () {

    //记录时间点
    var time = wgn.timeScroll ? wgn.timeScroll.time() : 0;
    if( wgn.timeScroll )   wgn.timeScroll.clear();

    wgn.timeScroll = new TimelineMax();
    //当从第二屏切换到第一屏的时候，让第二屏里面的动画时间点重归0
    wgn.timeScroll.to(".scene1",0,{onReverseComplete:function () {
        twoAnimate.timeline.seek(0,false);
    }},0);

    wgn.timeScroll.to(".footer",0,{top:"100%"});

    wgn.timeScroll.add(".step1");
    wgn.timeScroll.to(".scene2",0.8,{top : 0,ease:Cubic.easeInOut});
    wgn.timeScroll.to({},0.1,{onComplete : function () {
    ...
    wgn.timeScroll.to({},0,{onComplete:function () {
        twoAnimate.timeline.tweenTo("state1");
    }},"-=0.2");

    wgn.timeScroll.add(".step2");
    ...
    wgn.timeScroll.to({},0.4,{});//让整个动画持续0.4秒
    wgn.timeScroll.add("point1");
    ...
    wgn.timeScroll.to({},0.4,{});//让整个动画持续0.4秒
    wgn.timeScroll.add("point3");

    /*------主动画中配置第二屏的小动画 end----------*/

    wgn.timeScroll.to(".scene3",0.8,{top : 0,ease:Cubic.easeInOut,onReverseComplete:function () {
    ...
    wgn.timeScroll.to({},0,{onComplete:function () {
        ...
        twoAnimate.timeline.tweenTo("threeState1");
    }});
    wgn.timeScroll.to({},0.4,{});//让整个动画持续0.4秒
    wgn.timeScroll.add("threeState");

    /*------主动画中配置第三屏的小动画 end----------*/

    wgn.timeScroll.to(".scene4",0.8,{top : 0,ease:Cubic.easeInOut});
    wgn.timeScroll.add(".step4");

    //滚动到第五屏的时候，要让第四屏移出屏幕外面
    wgn.timeScroll.to(".scene4",0.8,{top:-$(window).height(),ease:Cubic.easeInOut});

    //当可视区域大于950，那么就让导航条隐藏起来
    if($(window).width() > 950){
        wgn.timeScroll.to(".menu_wrapper",0.8,{top:-110,ease:Cubic.easeInOut},"-=0.8");
    }else{
        $(".menu_wrapper").css("top",0);
    }

    wgn.timeScroll.to(".scene5",0.8,{top : 0,ease:Cubic.easeInOut,onReverseComplete:function () {
        fiveAnimate.timeline.seek(0,false);
    }},"-=0.8");
    wgn.timeScroll.to({},0.1,{onComplete:function () {
        fiveAnimate.timeline.tweenTo("fiveState");
    }},"-=0.2");
    wgn.timeScroll.add(".step5");

    wgn.timeScroll.to(".scene5",0.5,{top:-$(".footer").height(),ease:Cubic.easeInOut});
    wgn.timeScroll.to(".footer",0.5,{top:$(window).height()-$(".footer").height(),ease:Cubic.easeInOut},"-=0.5");

    wgn.timeScroll.add("footer");

    wgn.timeScroll.stop();
    //当改变浏览器的大小时，让动画走到之前已经到达的时间点
    wgn.timeScroll.seek(time);

};

//配置导航条的动画
wgn.configIntAnimate = function () {
...
//设置3D翻转效果
wgn.button3D = function(obj,element1,element2,d){
    ...
    button3DAnimate.to( $(obj).find(element2),0,{rotationX : -90,transformPerspective : 600,transformOrigin : "center top"} );
...
//设置img的百分比
wgn.imgWidthp = function(elementImg){
...
fiveAnimate.init = function () {
    //把所有的图片胡button翻转-90度，透明度变为0，.scene5_img的top初始为-220
...
menu.changeMenu = function ( stateClass ) {   //参数的作用是：切换到某一屏的时候，要传入的class的名字
    //具体实现3D翻转效果
    var oldMenu = $(".menu");
    ...
    wgn.nav();//执行导航条的鼠标移入移出的动画
    wgn.button3D(".start",".state1",".state2",0.3);//设置3D翻转效果

    var menuAnimate = new TimelineMax();

    //如果可视区域大于950，才能让导航条有一个3D的翻转效果过程
    if($(window).width() > 950 ){
